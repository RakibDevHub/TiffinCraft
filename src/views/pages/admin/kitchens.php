...
<div class="page-header dashboard-overview">
    ...
    <p class="page-subtitle" style="color: #fff;">Manage kitchen accounts, view performance metrics, and suspend
        kitchens</p>
</div>
...
<div class="dashboard-card">
    ...
    <div class="card-header pagination-header">
        <div class="pagination-info">
            Showing <?= ($page - 1) * $limit + 1 ?> to <?= min($page * $limit, $totalKitchens) ?> of
                            ...
                            <tr data-kitchen-id="<?= htmlspecialchars($kitchen['KITCHEN_ID'] ?? '') ?>"
                                    ...
                                    <div class="user-info">
                                        <?php if (!empty($kitchen['COVER_IMAGE'])): ?>
                                            <img src="/uploads/kitchen/<?= htmlspecialchars($kitchen['COVER_IMAGE']) ?>"
                                                class="user-avatar" alt="<?= htmlspecialchars($kitchen['KITCHEN_NAME'] ?? '') ?>">
                                        <?php else: ?>
                                            <img src="https://ui-avatars.com/api/?name=<?= urlencode($kitchen['KITCHEN_NAME'] ?? '') ?>&background=4a6cf7&color=fff"
                                                class="user-avatar" alt="<?= htmlspecialchars($kitchen['KITCHEN_NAME'] ?? '') ?>">
                                        <?php endif; ?>
                                        <div class="user-details" style="width: 100%;">
                                            ...
                                            <?php if (!empty($kitchen['SIGNATURE_DISH'])): ?>
                                                <small class="text-muted">Signature:
                                                    <?= htmlspecialchars($kitchen['SIGNATURE_DISH']) ?></small><br />
                                            <?php endif; ?>
                                            <?php if (!empty($kitchen['SERVICE_AREAS'])): ?>
                                                <small class="text-muted"
                                                    title="<?= htmlspecialchars($kitchen['SERVICE_AREAS']) ?>">
                                ...
                                <td>
                                    <?php if (!empty($kitchen['PLAN_NAME'])): ?>
                                        <div class="subscription-info">
                                            <span class="subscription-name"><?= htmlspecialchars($kitchen['PLAN_NAME']) ?> -
                                                ৳<?= htmlspecialchars($kitchen['MONTHLY_FEE']) ?></span>
                                            <span class="<?= $subscriptionClass ?>">
                                                <i
                                                    class="fas <?= $subscriptionStatus === 'ACTIVE' ? 'fa-check-circle' : 'fa-exclamation-circle' ?>"></i>
                                    ...
                                    <div class="status-info">
                                        <?php if ($isKitchenSuspended): ?>
                                            <div class="status-with-details">
                                                ...
                                                <?php if (!empty($kitchen['KITCHEN_SUSPENSION_REASON'])): ?>
                                                    <small class="text-muted"
                                                        title="<?= htmlspecialchars($kitchen['KITCHEN_SUSPENSION_REASON']) ?>">
                                        ...
                                        <?php else: ?>
                                            <span class="<?= $approvalClass ?>">
                                                <i
                                                    class="fas <?= ($kitchen['APPROVAL_STATUS'] ?? '') === 'approved' ? 'fa-check-circle' : 'fa-clock' ?>"></i>
                                            ...
                                            <?php if (($kitchen['APPROVAL_STATUS'] ?? '') === 'approved'): ?>
                                                <small class="text-muted" style="margin-top: 0.5rem;">
                                                    <i
                                                        class="fas <?= $subscriptionStatus === 'ACTIVE' ? 'fa-bolt text-success' : 'fa-power-off text-warning' ?>"></i>
                                ...
                                <td>
                                    <div class="performance-metrics">
                                        <div class="metric">
                                            <span
                                                class="metric-value"><?= htmlspecialchars($kitchen['TOTAL_ORDERS'] ?? '0') ?></span>
                                            <span class="metric-label">Orders</span>
                                        </div>
                                        <div class="metric">
                                            <span
                                                class="metric-value"><?= htmlspecialchars($kitchen['COMPLETED_ORDERS'] ?? '0') ?></span>
                                            <span class="metric-label">Delivered</span>
                                        </div>
                                        <div class="metric">
                                            <span
                                                class="metric-value"><?= htmlspecialchars($kitchen['TOTAL_MENU_ITEMS'] ?? '0') ?></span>
                                            <span class="metric-label">Menu Items</span>
                                        </div>
                                        <div class="metric">
                                            <span
                                                class="metric-value">৳<?= number_format($kitchen['CURRENT_BALANCE'] ?? 0, 2) ?></span>
                                ...
                                <td>
                                    <div class="action-buttons kitchen-table-btn">
                                        ...
                                        <button class="btn-action btn-view"
                                            onclick="viewKitchenDetails('<?= $kitchen['KITCHEN_ID'] ?>')" title="View Details">
        ...
        <form method="POST" action="/admin/dashboard/kitchens" id="suspendForm">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="reason" id="suspendReason" rows="4"
        ...
        <form method="POST" action="/admin/dashboard/kitchens" id="liftSuspensionForm">
            ...
            <div class="modal-body">
                <div class="confirmation-message">
                    ...
                    <p>You are about to lift the suspension for kitchen: <strong
                            id="liftSuspensionKitchenName"></strong></p>
...
<div class="modal-overlay" id="kitchenDetailsModal">
    <div class="modal modal-large">
        ...
        <div class="modal-body">
            <?php foreach ($kitchendata as $kitchen): ?>
                <div class="kitchen-details-content"
                    id="kitchen-details-<?= htmlspecialchars($kitchen['KITCHEN_ID'] ?? '') ?>" style="display: none;">
                                ...
                                <div class="kitchen-badges">
                                    ...
                                    <?php if (!empty($kitchen['COMPLETION_RATE'])): ?>
                                        <span class="status-badge status-success">
                                            <i class="fas fa-trophy"></i> <?= htmlspecialchars($kitchen['COMPLETION_RATE']) ?>%
                                            Completion
                                        </span>
                                    <?php endif; ?>
                                </div>
                        ...
                        <div class="kitchen-details-flex">
                            ...
                            <div class="detail-section">
                                ...
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value"><?= htmlspecialchars($kitchen['KITCHEN_NAME'] ?? 'N/A') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Description</div>
                                    <div class="detail-value">
                                        <?= !empty($kitchen['DESCRIPTION']) ? htmlspecialchars($kitchen['DESCRIPTION']) : 'No description provided' ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Address</div>
                                    <div class="detail-value"><?= htmlspecialchars($kitchen['KITCHEN_ADDRESS'] ?? 'N/A') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Signature Dish</div>
                                    <div class="detail-value">
                                        <?= !empty($kitchen['SIGNATURE_DISH']) ? htmlspecialchars($kitchen['SIGNATURE_DISH']) : 'Not specified' ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Experience</div>
                                    <div class="detail-value">
                                        <?= !empty($kitchen['YEARS_EXPERIENCE']) ? htmlspecialchars($kitchen['YEARS_EXPERIENCE']) . ' years' : 'Not specified' ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Average Prep Time</div>
                                    <div class="detail-value">
                                        <?= !empty($kitchen['AVG_PREP_TIME']) ? htmlspecialchars($kitchen['AVG_PREP_TIME']) . ' minutes' : 'Not specified' ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                            ...
                            <div class="detail-section">
                                ...
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value"><?= htmlspecialchars($kitchen['OWNER_EMAIL'] ?? 'N/A') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Phone</div>
                                    <div class="detail-value">
                                        <?= !empty($kitchen['OWNER_PHONE']) ? htmlspecialchars($kitchen['OWNER_PHONE']) : 'N/A' ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value">
                                        <span
                                            class="status-badge status-<?= strtolower($kitchen['OWNER_STATUS'] ?? 'active') ?>">
                                ...
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value">
                                        <?php if (!empty($kitchen['SERVICE_AREAS'])): ?>
                                            ...
                                            <small class="text-muted">(<?= htmlspecialchars($kitchen['TOTAL_SERVICE_AREAS']) ?>
                                                areas)</small>
                                        <?php else: ?>
                            ...
                            <div class="detail-section">
                                ...
                                <?php if (!empty($kitchen['PLAN_NAME'])): ?>
                                    ...
                                    <div class="detail-row">
                                        ...
                                        <div class="detail-value">
                                            ৳<?= !empty($kitchen['MONTHLY_FEE']) ? number_format($kitchen['MONTHLY_FEE'], 2) : '0.00' ?>
                                        </div>
                                    </div>
                                    <div class="detail-row">
                                        <div class="detail-label">Commission Rate</div>
                                        <div class="detail-value">
                                            <?= !empty($kitchen['COMMISSION_RATE']) ? htmlspecialchars($kitchen['COMMISSION_RATE']) . '%' : 'N/A' ?>
                                        </div>
                                    </div>
                                    <div class="detail-row">
                                        <div class="detail-label">Max Items</div>
                                        <div class="detail-value">
                                            <?= !empty($kitchen['MAX_ITEMS']) ? htmlspecialchars($kitchen['MAX_ITEMS']) : 'Unlimited' ?>
                                        </div>
                                    </div>
                                    <div class="detail-row">
                                        <div class="detail-label">Max Areas</div>
                                        <div class="detail-value">
                                            <?= !empty($kitchen['MAX_AREAS']) ? htmlspecialchars($kitchen['MAX_AREAS']) : 'Unlimited' ?>
                                        </div>
                                    </div>
                                    <div class="detail-row">
                                        ...
                                        <div class="detail-value">
                                            <span
                                                class="status-badge status-<?= strtolower($kitchen['SUBSCRIPTION_STATUS'] ?? 'inactive') ?>">
                            ...
                            <div class="detail-section">
                                ...
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value"><?= htmlspecialchars($kitchen['COMPLETED_ORDERS'] ?? '0') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Pending Orders</div>
                                    <div class="detail-value"><?= htmlspecialchars($kitchen['PENDING_ORDERS'] ?? '0') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Cancelled Orders</div>
                                    <div class="detail-value"><?= htmlspecialchars($kitchen['CANCELLED_ORDERS'] ?? '0') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value">
                                        <span
                                            class="<?= ($kitchen['COMPLETION_RATE'] ?? 0) >= 80 ? 'text-success' : (($kitchen['COMPLETION_RATE'] ?? 0) >= 60 ? 'text-warning' : 'text-danger') ?>">
                                ...
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value">৳<?= number_format($kitchen['TOTAL_EARNINGS'] ?? 0, 2) ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Platform Commission</div>
                                    <div class="detail-value">৳<?= number_format($kitchen['TOTAL_COMMISSION'] ?? 0, 2) ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Net Revenue</div>
                                    <div class="detail-value text-success">
                                        <strong>৳<?= number_format($kitchen['NET_EARNINGS'] ?? 0, 2) ?></strong>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Available Balance</div>
                                    <div class="detail-value text-primary">
                                        <strong>৳<?= number_format($kitchen['CURRENT_BALANCE'] ?? 0, 2) ?></strong>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Weekly Earnings</div>
                                    <div class="detail-value">৳<?= number_format($kitchen['WEEKLY_EARNINGS'] ?? 0, 2) ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Monthly Earnings</div>
                                    <div class="detail-value">৳<?= number_format($kitchen['MONTHLY_EARNINGS'] ?? 0, 2) ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                            ...
                            <div class="detail-section">
                                ...
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value"><?= htmlspecialchars($kitchen['TOTAL_REVIEWS'] ?? '0') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Total Menu Items</div>
                                    <div class="detail-value"><?= htmlspecialchars($kitchen['TOTAL_MENU_ITEMS'] ?? '0') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    <div class="detail-label">Available Menu Items</div>
                                    <div class="detail-value">
                                        <?= htmlspecialchars($kitchen['AVAILABLE_MENU_ITEMS'] ?? '0') ?>
                                    </div>
                                </div>
                                <div class="detail-row">
                                    ...
                                    <div class="detail-value">
                                        ...
                                        ?>
                                        <span
                                            class="<?= $availabilityRate >= 80 ? 'text-success' : (($availabilityRate >= 50 ? 'text-warning' : 'text-danger')) ?>">
                                            <?= $availabilityRate ?>%
                                        </span>
