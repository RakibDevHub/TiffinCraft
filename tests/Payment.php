...
class Payment
{
    ...
    public function subscriptionPayment($paymentData, $metadata = [])
    {
        try {
            ...
            error_log("seller_id: " . $paymentData['user_id']);
    ...
    public function handleSubscriptionPaymentCallback($postData, $getData)
    {
        ...
        try {
            ...
            try {
                ...
                $this->handleKitchenApproval($pendingTransaction);
    ...
    public function handleOrderPaymentCallback($postData, $getData)
    {
        ...
        try {
            ...
            try {
                ...
                $this->clearUserCart($pendingTransaction['user_id']);
    ...
    private function recordTransaction($transactionData)
    {
        try {
            ...
            $amount = (float) $transactionData['amount'];  // This will be cast to NUMBER(10,2)
            ...
            // Handle reference_id - can be null for failed transactions
    ...
    private function handleKitchenApproval($pendingTransaction)
    {
        if (
            $pendingTransaction['metadata']['subscription_type'] === 'NEW'
            && !empty($pendingTransaction['metadata']['kitchen_id'])
        ) {
            $this->approveKitchen($pendingTransaction['metadata']['kitchen_id']);
        }
    }
    ...
    private function createOrderRecord($orderData)
    {
        try {
            ...
            // This method should handle stock validation and updates internally
            $orderId = $orderModel->createOrder($orderData, $orderData['cart_items']);
    ...
    private function restoreOrderStock($orderId)
    {
        try {
            $orderModel = new Order($this->conn);
            return $orderModel->restoreStock($orderId);
        } catch (Exception $e) {
    ...
    private function approveKitchen($kitchenId)
    {
        $updateSql = "UPDATE kitchens SET approval_status = 'approved', updated_at = SYSTIMESTAMP WHERE kitchen_id = :kitchen_id";
        $updateStmt = oci_parse($this->conn, $updateSql);
        oci_bind_by_name($updateStmt, ':kitchen_id', $kitchenId);
        return oci_execute($updateStmt, OCI_NO_AUTO_COMMIT);
    }
    ...
    {
        $sql = "SELECT * FROM payment_transactions WHERE transaction_id = :transaction_id";
        $stmt = oci_parse($this->conn, $sql);
        oci_bind_by_name($stmt, ':transaction_id', $transactionId);
        oci_execute($stmt);
    ...
    public function updatePaymentStatus($transactionId, $status, $valId = null)
    {
